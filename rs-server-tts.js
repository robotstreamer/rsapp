...
tts.now = function(phrase, callback) {
  ...
  if (tts.voice === 'espeak') {
    ...
    let speakProcess = exec.quiet(espeakCommand + ' -s ' + parseInt(store.get('config.rs.tts_speed')*100)+' '+store.get('config.rs.tts_extra')+' "'+phrase.replace(/\"/g, '|')+'"', function(err,res){
